...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      "fixed inset-0 z-50 bg-black/50",  // ปรับเป็นสีเข้มแบบโปร่งแสง
      className
    )}
    {...props}
  />
...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-[50%] top-[50%] z-50 grid w-[70%] max-w-xs translate-x-[-50%] translate-y-[-50%] gap-4 border bg-yellow-50 p-0 shadow-md rounded-lg",  // ปรับขนาดให้เล็กลง และโทนสีเหลือง
        className
      )}
      {...props}
    >
