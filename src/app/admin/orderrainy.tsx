...
const RainyOrdersPage = () => {
  const [orders, setOrders] = useState<any[]>([]);

  useEffect(() => {
    const storedOrders = JSON.parse(localStorage.getItem("orders") || "[]");
    const rainyOrders = storedOrders.filter((order: any) => order.status === "Delayed due to rain");
    setOrders(rainyOrders);
  }, []);
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">ออเดอร์ที่ฝนตก</h1>
      <DataTable columns={columns} data={orders} pagination />
    </div>
  );
};

export default RainyOrdersPage;
